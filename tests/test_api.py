...
def test_list_job_repo(app, auth_header, userinfo, mock_db, monkeypatch, requests_mock):
    ...
    def mock_list_files(cls, machine, target_path):
        return [
            {'group': 'root', 'last_modified': '2022-12-02T09:17:51', 'link_target': '', 'name': 'e1000', 'permissions': 'rwxr-xr-x', 'size': '20480', 'type': 'd', 'user': 'root'}, 
            {'group': 'root', 'last_modified': '2022-12-02T16:00:40', 'link_target': '', 'name': 'snx3000', 'permissions': 'rwxr-xr-x', 'size': '262144', 'type': 'd', 'user': 'root'},
            {'group': 'user', 'last_modified': '2022-12-03T09:17:51', 'link_target': '', 'name': '_job.sh', 'permissions': 'rwxr-xr-x', 'size': '20480', 'type': 'f', 'user': 'userA'}, 
        ]
    
    # monkeypatch the mkdir/submit/cancel operation of f7t
    monkeypatch.setattr('firecrest.Firecrest.mkdir', MethodType(mock_mkdir, Firecrest))
    monkeypatch.setattr('firecrest.Firecrest.submit', MethodType(mock_submit, Firecrest))
    monkeypatch.setattr('firecrest.Firecrest.list_files', MethodType(mock_list_files, Firecrest))
...
def test_file_operations_repo(app, auth_header, userinfo, mock_db, monkeypatch, requests_mock):
    """This test launch job to cluster and get job list by f7t."""
    ...
    monkeypatch.setattr('firecrest.Firecrest.mkdir', MethodType(mock_mkdir, Firecrest))
    monkeypatch.setattr('firecrest.Firecrest.submit', MethodType(mock_submit, Firecrest))
    monkeypatch.setattr('hpc_gateway.model.f7t.Firecrest.simple_upload', MethodType(mock_simple_upload, Firecrest))
    monkeypatch.setattr('firecrest.Firecrest.simple_delete', MethodType(mock_simple_delete, Firecrest))
    monkeypatch.setattr('firecrest.Firecrest.simple_download', MethodType(mock_simple_download, Firecrest))
    ...
    response = client.get(f"/api/v1/file/download/{job_id}/{_dummy_filename}", headers=auth_header)
