...
from flask import Flask, flash, g, jsonify, send_file
from flask import Response
from werkzeug.utils import secure_filename

from gateway.auth_middleware import token_required
from gateway.f7t import Firecrest, HardCodeTokenAuth
from gateway.model import Jobs, User
...
def heartbeat(current_user):
    ...
    if not user:
        exc = f"Can not find user {user} in DB, please register by create user endpoint."
        msg = (
        ...
        try:
            # trigger a list_files operation by f7t, if running without exception
            # means the connect to cluster is ready.
            f7t_client.list_files(machine=MACHINE, target_path=target_path)
            ...
            return Response(
                "HPC-gatway-App : HPC system is running and ready.",
                status=200,
                mimetype="text/plain",
            )
...
def get_user(current_user):
    ...
    if user:
        repo = email2repo(current_user['email'])
        msg = (
            f"HPC-gatway-App : DB {user} mapping to markectplace user: {current_user}, "
            f"with file repository {repo}."
        )
        return Response(
            msg,
            status=200,
        ...
        msg = (
            ...
            f"Error message: User {current_user} not registered."
        )
        logging.error(msg)
        return Response(str(msg), status=400, mimetype="text/plain")
